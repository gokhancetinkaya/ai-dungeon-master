#!/usr/bin/env python3
"""
Download text generation model from HuggingFace for Triton deployment
"""
...
def download_model(model_name: str, output_dir: str):
    """Download Mistral 7B model and tokenizer from HuggingFace"""
    try:
    ...
    print("\n[1/1] Downloading model and tokenizer files...")
    ...
    for attempt in range(max_retries):
        try:
            ...
            # Use snapshot_download() instead of AutoTokenizer/AutoModelForCausalLM.from_pretrained()
            # This downloads files directly without loading the model into RAM.
            # Loading a 14GB model with from_pretrained() requires 20-30GB RAM and causes OOM.
...
def main():
    parser = argparse.ArgumentParser(description="Download text generation model for Triton")
    parser.add_argument(
        "--model",
        type=str,
