#!/bin/bash
# Deploy AI Dungeon Master with Single GPU Setup
...
echo "=========================================="
echo "Deploying AI Dungeon Master (Single GPU)"
echo "=========================================="
...
# Deploy combined Triton server with hostPath mounts
echo ""
echo "[4/7] Deploying combined Triton inference server (1 GPU)..."

# Deploy Triton combined server
envsubst < "$PROJECT_DIR/kubernetes/triton-combined-deployment.yaml" | microk8s kubectl apply -f -
echo "✓ Triton combined server deployed (models mounted from host)"

# Wait for Triton pod to be ready
echo ""
echo "[5/7] Waiting for Triton pod to load models (this may take a few minutes)..."
microk8s kubectl wait --for=condition=ready pod -l app=triton-combined -n ai-dungeon-master --timeout=300s || {
    echo "WARNING: Triton pod not ready yet. Checking status..."
    microk8s kubectl get pods -n ai-dungeon-master -l app=triton-combined
    microk8s kubectl describe pod -n ai-dungeon-master -l app=triton-combined | tail -20
    echo "Continuing with deployment..."
}
echo "✓ Triton pod is ready and serving models"
...
microk8s kubectl wait --for=condition=ready pod -l app=triton-combined -n ai-dungeon-master --timeout=600s || echo "⚠ Triton pods not ready yet"
